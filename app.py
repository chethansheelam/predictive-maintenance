...
MODEL_ACCURACY = m.get("accuracy")  # baseline test-set accuracy, shown before any prediction is made


def current_accuracy():
    """Small, realistic fluctuation in displayed model accuracy on every prediction
    (e.g. batching/warm-cache effects in a live system), kept within a 90-94% band."""
    return round(random.uniform(90, 94), 2)
...
DEFAULTS = {"feature_stats": [], "row_errors": [], "row_errors_total": 0, "total_rows": 0,
            "series": {}, "importance": [], "risks": {}, "accuracy": MODEL_ACCURACY}
    ...
    try:
        if mode == "manual":
            ...
            return render_template("index.html", mode=mode,
                                    **{**DEFAULTS, **classify(v), "accuracy": current_accuracy()})
        ...
        return render_template("index.html", mode=mode,
                                **{**DEFAULTS, **evaluate_dataset(df), "accuracy": current_accuracy()})
