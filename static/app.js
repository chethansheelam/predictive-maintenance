...
if(featureStats){
  ...
  featureStats.forEach(f=>{
    const canvas=document.getElementById("trend_"+f.key.replace(/[^a-zA-Z0-9_]/g,""));
    if(!canvas)return;
    // Keep the time-series chart robust for large CSVs and missing/non-numeric values.
    const vals=(series[f.key]||[]).map(v=>Number(v)).filter(v=>Number.isFinite(v));
    ...
    new Chart(canvas,{
      ...
      data:{labels:labels,datasets:[
        {label:f.label,data:vals,borderColor:"#4FA8E0",backgroundColor:"transparent",borderWidth:1.5,tension:.15,
         pointRadius:vals.map(v=>v>Number(f.threshold)?2:0),pointBackgroundColor:"#E5484D",pointBorderColor:"#E5484D",spanGaps:true},
        {label:"Limit",data:vals.map(()=>Number(f.threshold)),borderColor:"#E8A33D",borderDash:[5,4],borderWidth:1.5,pointRadius:0}
      ]},
      options:{responsive:true,maintainAspectRatio:false,animation:false,plugins:{legend:{display:false}},
        scales:{x:{display:false},y:{beginAtZero:false,grid:{color:"#D9E0E7"}}}}
    });
  });
}
